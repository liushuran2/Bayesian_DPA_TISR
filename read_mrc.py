...
if __name__ == '__main__':
    mrc_path = '/Ensconsin' # location of downloaded MRC file
    save_path = '/Ensconsin_h5' # saving path
    cell_count_valid = 5 # number of cells for validation
    SNR_flag = 1 #SNR level = [1,2,3](2D) | [1,2](3D)

    n_frame = 7 # number of the input frame length
    patchsize = 512 # patchsize of the dataset
    factor = 2 # super resolution factor

    outputfile_train = os.path.join(save_path, 'train.h5')
    outputfile_valid = os.path.join(save_path, 'valid.h5')


    filelist = FindAllSuffix(mrc_path, '.mrc') # all mrc files
    cell_count = count_subfolders(mrc_path) # number of cells
    num_of_SNR = len(filelist) // cell_count - 1 # number of low resolution SNR
    ...
    # VALID
    h5_file = h5py.File(outputfile_valid, 'w')
    lr_group = h5_file.create_group('lr')
    hr_group = h5_file.create_group('hr')
    patch_idx = 0

    for i in range(cell_count_valid):
        lr_file = filelist[i * (num_of_SNR + 1) + SNR_flag - 1]
        lr_in = readmrc_rawSIM(lr_file, img_size=patchsize, timepoints=n_frame)
